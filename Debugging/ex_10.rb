=begin
A friend of mine wants to build a basic search engine and
starts reading up on information extraction.  He decides to
try a simple implementation of TF-IDF (term frequency-inverse
document frequency), a measure of the information content a
term holds for a particular document in a document collection.
However, he just started learning to code and somehow his
numbers do not match what is expected. He knows I'm enrolled
in an awesome developer school, he asks me to have a look.
=end
...
=begin
In the definition of our original `idf` method, we perform
division on two values which will both be integers. The 
division will therefore be performed as *integer division*,
meaning it will produce an integer value, namely the integer
part of the result without the remainder (for example, `1/2`
will evaluate to `0` instead of `0.5`).

Let's take a closer look at our first test case. The term 
`"cat"` occurs in 2 of 3 documents, which return an `idf`
value of `Math.log(3/2) => Math.log(1) => 0`.

In order to return a float value, we must perform *float
division*. We can do this by either converting one or both
values to a floating point number using `Integer#to_f`, or by
using the `Numeric#fdiv` method as seen in the above solution.

The `idf` value of `"cat"` is then correctly calculated as 
`Math.log(3.fdiv(2)) => Math.log(1.5) => 0.405465108108644`.
The `tfidf` method multiplies the returned `idf` by the `tf`
value for `document1` (which is 3, because `"cat" occurs three
times in that document) and returns the expected output of 
~1.2 for the first example.

There's still one issue that could get us into trouble when
performing division. If you don't see it immediately, remove
`document1` from the `documents` collection and check the 
TF-IDF value of `'quantum'` again.
=end